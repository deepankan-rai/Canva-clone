...
const TemplateDesign = ({type}) => {
  ...
  return (
    <>
        ...
        {
        templates.map((design,i)=> <div onClick={()=>add_template(design._id)} className={`relative cursor-pointer group w-full h-[150px]  ${type ? 'h-[100px]' : 'h-[170px] px-2' }`} >
        <div className={`w-full h-full block bg-[#fffff12] rounded-md ${type ? '' : 'p-4'}`}>
